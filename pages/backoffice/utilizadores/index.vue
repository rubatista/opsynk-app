...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <BaseButton to="/backoffice/utilizadores/novo">+ Novo Utilizador</BaseButton>
    </div>

    <p v-if="!users?.length" class="text-gray-500">Ainda não há utilizadores.</p>
      ...
      <tbody>
        <tr v-for="user in users" :key="user.id" class="border-t border-gray-100">
          ...
          <td class="px-4 py-3 text-right space-x-3">
            <NuxtLink :to="`/backoffice/utilizadores/${user.id}`" class="text-indigo-600 hover:underline">
              Editar
            </NuxtLink>
